...
def p_start(p):
    ...
    p[0] = ('start', get_production(p))

def p_program_statement(p):
    '''program_statement : KEYWORD_PROGRAM IDENTIFIER SEMICOLON
                         | empty'''
    p[0] = ('program_statement', get_production(p))
...
def p_value_parameter(p):
    ...
    p[0] = ('value_parameter', get_production(p))

def p_block(p):
    'block : KEYWORD_BEGIN statements KEYWORD_END'
    p[0] = ('block', get_production(p))

def p_statements(p):
    '''statements : statements SEMICOLON statement
                  | statement'''
    p[0] = ('statements', get_production(p))

def p_statement(p):
    '''statement : matched_statement
                 | unmatched_statement'''
    p[0] = ('statement', get_production(p))
...
def p_matched_statement(p):
    '''matched_statement : simple_statement
                         ...
                         | empty'''
    p[0] = ('matched_statement', get_production(p))
...
def p_unmatched_statement(p):
    ...
    p[0] = ('unmatched_statement', get_production(p))

def p_loop_header(p):
    '''loop_header : for_loop_header
                   | while_loop_header'''
    p[0] = ('loop_header', get_production(p))
...
def p_for_loop_header(p):
    ...
    p[0] = ('for_loop_header', get_production(p))

def p_while_loop_header(p):
    '''while_loop_header : KEYWORD_WHILE expression KEYWORD_DO'''
    p[0] = ('while_loop_header', get_production(p))

def p_simple_statement(p):
    '''simple_statement : assignment_statement
                        | func_proc_statement'''
    p[0] = ('simple_statement', get_production(p))

def p_assignment_statement(p):
    '''assignment_statement : IDENTIFIER COLON_EQUAL expression'''
    p[0] = ('assignment_statement', get_production(p))

def p_expression(p):
    '''expression : simple_expression relational_operator simple_expression
                  | simple_expression'''
    p[0] = ('expression', get_production(p))
...
def p_simple_expression(p):
    '''simple_expression : term OP_PLUS simple_expression
                         ...
                         | term'''
    p[0] = ('simple_expression', get_production(p))
...
def p_variable_reference(p):
    ...
    p[0] = ('variable_reference', get_production(p))

def p_array_index(p):
    '''array_index : array_index COMMA expression
                   | expression COMMA expression'''
    p[0] = ('array_index', get_production(p))
...
def p_array_index_cstyle(p):
    ...
    p[0] = ('array_index_cstyle', get_production(p))

def p_sign(p):
    '''sign : OP_PLUS
            | OP_MINUS'''
    p[0] = ('sign', get_production(p))
...
def p_unsigned_constant(p):
    '''unsigned_constant : CONSTANT_INTEGER
                         ...
                         | string'''
    p[0] = ('unsigned_constant', get_production(p))
...
def p_structured_statement(p):
    '''structured_statement : block
                            | repeat_statement'''
    p[0] = ('structured_statement', get_production(p))

def p_repeat_statement(p):
    '''repeat_statement : KEYWORD_REPEAT statements KEYWORD_UNTIL expression'''
    p[0] = ('repeat_statement', get_production(p))

def p_empty(p):
    'empty :'
    p[0] = ('empty', get_production(p))
